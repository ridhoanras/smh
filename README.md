...
The title, text and image in the `Kurikulum` section can be customized in the markdown file `src/markdown/kurikulum.md`. The image for this section should be placed in the `src/images` folder and its path specified as `../images/[your-picture.png]`. The image should be square and at least 1050 x 1050 pixels and either PNG or JPG. The image will be automatically converted to a circle.

This section is optional, as is the section title and image. Delete or rename the `kurikulum.md` file if you do not want this section or delete the title and/or image fields if you do not want them.

### Stage subsection

The stage subsections are declared in the `src/markdown/stage` folder. You should create one file for each subsection with an optional title and image. You must specify the order of each subsection in its corresponding file.

The images for each subsection should be placed in the `src/images/stage` folder and its path specified as `../../images/stage/[your-picture.png]`. The image should be square and at least 900 x 900 pi and either PNG or JPG.
...
The text in this section can be customized in the markdown file `src/markdown/registration.md`.

This section is optional. Delete or rename the `registration.md` file if you do not want it.
...
The about page is built from markdown files in `src/markdown/about` and images in `src/images/about`. Each person in the lab should have his/her own file in the markdown folder. The person's name and order of appearance are required, while the e-mail, image and title fields are optional.

Images should be square and at least 900 x 900 pixels and either PNG or JPG.

This page is optional. Delete all files in this folder if you do not want this page. This will also remove the `About` link from the navigation bar.

### Blog page

Blog for this page are automatically retrieved from PubMed via PMIDs specified in the `gatsby-config.js` file. They will be ordered by blog date. The IDs should be placed in the `blog` field in the `siteMetadata` object.

This page is optional. Delete all links from the array or delete the field to remove the page. This will also remove the `Blog` link from the navigation bar.
